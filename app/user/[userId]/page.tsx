...
import { LinkIcon, User, Activity } from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6 md:p-12">
      <Suspense fallback={<UserSkeleton />}>
        <UserProfileFeed userId={userId} />
      </Suspense>
    </main>
  );
}

async function UserProfileFeed({ userId }: { userId: string }) {
  // Stats and the archive are independent — fetch them concurrently.
  ...
  const username = stats.username || "Whizz";
    ...
    <div className="max-w-7xl mx-auto space-y-12">
      ...
      <div className="bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-2xl p-8 md:p-12 text-center md:text-left shadow-2xl">
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          {/* Avatar */}
          <div className="w-24 h-24 bg-gray-800 rounded-full flex items-center justify-center border-4 border-gray-700 shadow-xl shrink-0">
          ...
          <div className="space-y-2 flex-1 w-full md:w-auto">
            ...
            <p className="text-gray-500 text-sm">
              {stats.totalSolved} problems solved · {stats.uniqueDays} active days
            </p>
          </div>

          {/* STATS GRID: Problems | Days | Cur Streak | Max Streak */}
      ...
      <div className="space-y-6">
        ...
        <Archive data={userData} currentUserId={null} />
      </div>
